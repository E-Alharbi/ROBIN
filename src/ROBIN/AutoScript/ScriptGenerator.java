...
public class ScriptGenerator {
	...
	public  String generatescript(String Pipeline) throws IOException {
		...
		String chltofom="";	
		String PhasesCopy=Parameters.getPhases();
		String ParrotPhasesCopy=Parameters.getParrotPhases();
		
		if(Parameters.getPhases().equals(Parameters.getParrotPhases()) && Parameters.getPhases().split(",").length<4) {
			chltofom+="\n chltofom -mtzin inputdata/"+ Parameters.getReflectionFileName()+".mtz -mtzout inputdata/hltophi.mtz -colin-phifom "+Parameters.getParrotPhases()+" -colout robin \n";
			chltofom+="rm inputdata/"+Parameters.getReflectionFileName()+".mtz \n";
			chltofom+="mv inputdata/hltophi.mtz inputdata/"+Parameters.getReflectionFileName()+".mtz \n";
			Parameters.setParrotPhases("robin.ABCD.A,robin.ABCD.B,robin.ABCD.C,robin.ABCD.D");
			Parameters.setPhases("robin.ABCD.A,robin.ABCD.B,robin.ABCD.C,robin.ABCD.D");

		}
		else {
		if(Parameters.getPhases().split(",").length<4) {
			chltofom+="\n chltofom -mtzin inputdata/"+ Parameters.getReflectionFileName()+".mtz -mtzout inputdata/hltophi.mtz -colin-phifom "+Parameters.getPhases()+" -colout robin \n";
			chltofom+="rm inputdata/"+Parameters.getReflectionFileName()+".mtz \n";
			chltofom+="mv inputdata/hltophi.mtz inputdata/"+Parameters.getReflectionFileName()+".mtz \n";
			Parameters.setPhases("robin.ABCD.A,robin.ABCD.B,robin.ABCD.C,robin.ABCD.D");
		}
		if(Parameters.getParrotPhases().split(",").length<4) {
			chltofom+="\n chltofom -mtzin inputdata/"+ Parameters.getReflectionFileName()+".mtz -mtzout inputdata/hltophi.mtz -colin-phifom "+Parameters.getParrotPhases()+" -colout robinparrot \n";
			chltofom+="rm inputdata/"+Parameters.getReflectionFileName()+".mtz \n";
			chltofom+="mv inputdata/hltophi.mtz inputdata/"+Parameters.getReflectionFileName()+".mtz \n";
			Parameters.setParrotPhases("robinparrot.ABCD.A,robinparrot.ABCD.B,robinparrot.ABCD.C,robinparrot.ABCD.D");
		...
		if(Parameters.getMR().equals("T")) {
			...
			MRHeader="if [ ! -f "+Parameters.getReflectionFileName()+".pdb ]; then\n"
					...
					+ "    exit 3\n"
					+ "fi\n";
			MRHeader+= "cp "+Parameters.getReflectionFileName()+".pdb inputdata \n";
		...
		Header+="\n maindir=$(pwd)\n"
				...
				+ "\n"
				+ "mkdir inputdata\n"
				+ "cp "+Parameters.getReflectionFileName()+".mtz inputdata\n"
				+ "cp "+Parameters.getReflectionFileName()+".fasta inputdata";
			...
			Parameters.setPhases(PhasesCopy);// Reset phases to avoid the overwriting problem when this function call again on another pipeline.
			Parameters.setParrotPhases(ParrotPhasesCopy);
